...
const CategoryCard = ({ category }) => {
    ...
    return (
        <NavLink to={`/products/${categoryName}`} id={_id} title={description} className="w-60 h-full pb-2 bg-gray-700 border shadow-[rgba(7,_65,_210,_0.1)_0px_9px_30px] border-gray-400 transition-all duration-200 rounded-lg cursor-pointer backdrop-filter backdrop-blur-md hover:bg-gray-600 hover:bg-opacity-60 bg-opacity-60"
        >
            <div className="relative">
                <img src={imgUrl} className='max-w-xs p-2 h-60 w-60'  alt={description} />
