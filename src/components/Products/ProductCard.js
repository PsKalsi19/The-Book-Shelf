...
const ProductCard = ({ product,fromWishlist }) => {
  ...
  const {handleWishlistToggle,removeWishlistHandler}=useContext(BooksContext)
  ...
  return (
    <div className="flex self-start flex-col items-center border border-gray-900 rounded-lg hover:bg-gray-800 hover:border hover:border-gray-700 ">
      <div className="relative">
        <img className="w-40 h-56 p-4 rounded-t-lg lg:w-56 lg:h-80" src={imgUrl} alt={title} />
       {!fromWishlist && <button type="button" onClick={(e)=> wishlistHandler(e,product)} className="absolute right-0 w-12 h-12 text-pink-600 rounded-full top-1">
          <svg
       ...
       {fromWishlist && <button type="button" onClick={(e)=> removeFromWishLIst(e,product)} className="absolute right-0 w-12 h-12 text-pink-600 rounded-full top-1">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
